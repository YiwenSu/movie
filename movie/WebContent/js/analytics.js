(function(){
	...
	var tracker = {
			// config
			clientConfig: {
				serverUrl: "http://localhost/oracle.jpg",
			...
			columns: {
				...
				eventName: "en",//时间名称
				version: "ver",//版本
				platform: "pl",//平台，java_server/website
				sdk: "sdk",//使用的sdk，java/js
				uuid: "u_ud",//用户id
				memberId: "u_mid",//会员id
				sessionId: "u_sd",//会话id
				...
				language: "l",//语言
				userAgent: "b_iev",//头信息
				resolution: "b_rst",//分辨率
				currentUrl: "p_url",//当前url
				referrerUrl: "p_ref",//上一个url
				title: "tt",//网页标题
				orderId: "oid",//订单id
				orderName: "on",//订单名称
				currencyAmount: "cua",//订单金额
				currencyType: "cut",//现金类型
				paymentType: "pt",//支付方式
				category: "ca",//事件category
				action: "ac",//事件action
				kv: "kv_",//自定义事件
				duration: "du"//持续时间
			},
			...
			keys: {
				...
				sid: "oracle_sid", //会话id
				uuid: "oracle_uuid",//每个用户一个固定的uuid，用户标识符
				mid: "oracle_mid",//会员id，和业务中的用户id一致
				preVisitTime: "oracle_previsit",//最近访问时间，可以用来检测session是否过期
			...
			startSession: function() {
				// 加载js就触发的方法
				//if (this.getSid()) {
				if(window.sessionStorage[this.keys.sid]) {
					// 会话id存在，表示uuid也存在
			...
			createNewSession: function() {
				...
				window.sessionStorage[this.keys.sid] = sid;
				this.setSid(sid);
				...
				CookieUtil.set(tracker.keys.mid, null, 0, "/");
			},
